...
if ($pesanan_id === false || $pesanan_id <= 0) {
    ...
    if (isset($_SESSION['role'])) {
        switch ($_SESSION['role']) {
            case 'admin': $redirect_page = '../admin/manage_orders.php'; break;
            case 'pelanggan': $redirect_page = '../pelanggan/track_delivery.php'; break;
            case 'sopir': $redirect_page = '../sopir/my_deliveries.php'; break;
        }
    }
    header("Location: " . $redirect_page);
    exit;
} else {
    // --- Query diperbarui untuk mengambil no_kendaraan ---
...
$current_role = $_SESSION['role'] ?? 'pelanggan';
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logout-btn a { background: linear-gradient(135deg, #ff6b6b, #ee5a5a); color: white !important; border-radius: 8px; border-left: none !important; }
        .logout-btn a:hover { background: linear-gradient(135deg, #ff5252, #d32f2f); transform: translateY(-2px) !important; box-shadow: 0 5px 15px rgba(255, 107, 107, 0.4); }

        .main-content { margin-left: 280px; flex-grow: 1; padding: 40px; min-height: 100vh; }
        .content-card { background: rgba(255, 255, 255, 0.97); backdrop-filter: blur(10px); border-radius: 20px; padding: 30px 40px; box-shadow: 0 20px 40px rgba(0, 0, 0, 0.1); border: 1px solid rgba(255, 255, 255, 0.2); }
        .content-card h2 { font-size: 2.2rem; margin-bottom: 25px; background: linear-gradient(135deg, #667eea, #764ba2); background-clip: text; -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-align: center; }
        .section { margin-bottom: 25px; }
        .section h3 { font-size: 1.4rem; color: #333; border-bottom: 2px solid #ddd; padding-bottom: 10px; margin-bottom: 20px; display: flex; align-items: center; }
        ...
        .btn { background-color: #3498db; color: white !important; padding: 12px 20px; border-radius: 8px; text-decoration: none; border: none; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.1); font-weight: 500; display: inline-flex; align-items: center; }
        ...
        .status-badge.dibatalkan { background-color:rgb(216, 25, 25); }
        ...
        @media (max-width: 768px) {
            ...
            .mobile-toggle, .overlay.active { display: block; }
...
<body class="theme-<?php echo htmlspecialchars($current_role); ?>">
    ...
    <!-- [PERBAIKAN]: Elemen div overlay yang hilang sekarang ditambahkan -->
    <div class="overlay" id="overlay"></div>

    <div class="sidebar" id="sidebar">
        <!-- Konten Sidebar (tidak diubah) -->
    ...
    <main class="main-content" id="main-content">
        <div class="content-card">
            ...
            <?php if ($order_details): ?>
                <!-- Informasi Pesanan (tidak diubah) -->
                <div class="section">
                ...
                <!-- Rincian Item Pesanan (tidak diubah) -->
                <div class="section">
                ...
                <div class="section">
                    ...
                    <?php if (!empty($order_details['pengiriman_id_data'])): ?>
                        ...
                        <p>No. Kendaraan: <strong><?php echo htmlspecialchars($order_details['no_kendaraan'] ?? 'Belum ada info'); ?></strong></p>
                        <p>Tanggal Dikirim: <strong><?php echo !empty($order_details['tanggal_kirim']) ? date('d F Y', strtotime($order_details['tanggal_kirim'])) : 'Menunggu update dari sopir'; ?></strong></p>
                        ...
                        <?php 
                        $latIsValid = isset($order_details['koordinat_sopir_lat']) && (float)$order_details['koordinat_sopir_lat'] != 0;
                        $longIsValid = isset($order_details['koordinat_sopir_long']) && (float)$order_details['koordinat_sopir_long'] != 0;
                        ...
                        <?php else: ?>
                            <div class="location-placeholder">
                                ...
                                <p>Lokasi sopir akan ditampilkan di sini setelah sopir memulai perjalanan dan memperbarui lokasinya.</p>
                            </div>
                        <?php endif; ?>
    ...
    <script>
        ...
        const overlay = document.getElementById('overlay'); // Sekarang tidak akan null
        ...
        overlay.addEventListener('click', closeSidebar); // Sekarang akan berfungsi
        document.addEventListener('keydown', (e) => { if (e.key === "Escape" && sidebar.classList.contains('active')) closeSidebar(); });
        window.addEventListener('resize', () => { if (window.innerWidth > 768) closeSidebar(); });

        // Script Peta, hanya akan dijalankan jika PHP memberikan bloknya
        <?php 
        if ($order_details) {
            $latIsValid = isset($order_details['koordinat_sopir_lat']) && (float)$order_details['koordinat_sopir_lat'] != 0;
            $longIsValid = isset($order_details['koordinat_sopir_long']) && (float)$order_details['koordinat_sopir_long'] != 0;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            try {
                ...
                document.getElementById('delivery-map').innerHTML = '<p>Gagal memuat peta.</p>';
            }
        });
        <?php 
            }
