...
$order_data = null;
$sopir_list = []; // Akan berisi daftar sopir yang TERSEDIA
$message = '';
$message_type = '';
...
// Query untuk mengambil data pesanan (tidak ada perubahan)
...
// --- DIUBAH: Query untuk mengambil daftar sopir yang TERSEDIA ---
// Kita hanya akan mengambil sopir yang tidak memiliki pengiriman dengan status 'dalam perjalanan'.
$query_sopir = "
    SELECT u.id, u.nama_lengkap
    FROM users u
    LEFT JOIN pengiriman p ON u.id = p.id_sopir AND p.status_pengiriman = 'dalam perjalanan'
    WHERE u.role = 'sopir' AND p.id IS NULL
    ORDER BY u.nama_lengkap ASC
";
$stmt_sopir = $conn->prepare($query_sopir);
$stmt_sopir->execute();
...
// Logika untuk memproses form (tidak ada perubahan)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['assign_driver'])) {
    ...
    $status_pengiriman_awal = 'ditugaskan';
...
<html lang="id">
<head>
    <!-- ... Bagian Head, CSS, dan HTML lainnya tetap sama persis ... -->
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Tugaskan Pengiriman - Admin</title>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <style>
        /* CSS Anda di sini (tidak ada perubahan) */
...
<body>
    <div class="sidebar" id="sidebar">
        <!-- Sidebar HTML Anda (tidak berubah) -->
    ...
    <div class="main-content">
        ...
        <div class="form-container">
            <!-- Tampilan pesan dan detail pesanan (tidak ada perubahan) -->
            ...
            <?php if ($order_data): ?>
                ...
                <form action="assign_delivery.php?pesanan_id=<?php echo htmlspecialchars($pesanan_id); ?>" method="POST">
                    <div class="form-group">
                        <label for="sopir_id"><i class="fas fa-id-card"></i> Pilih Sopir Pengirim (Hanya yang Tersedia)</label>
                        <select id="sopir_id" name="sopir_id" required>
                            ...
                            <?php if (empty($sopir_list)): ?>
                                <option value="" disabled>Tidak ada sopir yang tersedia saat ini</option>
                            <?php else: ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="tanggal_kirim" name="tanggal_kirim" value="<?php echo date('Y-m-d'); ?>" min="<?php echo date('Y-m-d'); ?>" required>
                    </div>
                    ...
                    <div class="button-group">
                        ...
                        <button type="submit" name="assign_driver" class="btn btn-primary" <?php echo empty($sopir_list) ? 'disabled' : ''; ?>><i class="fas fa-check-circle"></i> Tugaskan</button>
                    </div>
                </form>
            <?php endif; ?>
        </div>
