...
if (!$pesanan_id) {
    ...
    $redirect_page .= ($_SESSION['role'] === 'admin') ? 'manage_deliveries.php' : 'order_history.php'; 
...
// --- Ambil Detail Pesanan (Query diperbarui untuk menyertakan info pembayaran) ---
...
$can_view = false;
if ($_SESSION['role'] === 'admin' || ($_SESSION['role'] === 'pelanggan' && $order_details['id_pelanggan_owner'] == $_SESSION['user_id']) || ($_SESSION['role'] === 'sopir' && !empty($order_details['id_sopir_owner']) && $order_details['id_sopir_owner'] == $_SESSION['user_id'])) {
    $can_view = true;
}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-header { background: white; border-radius: 20px; padding: 30px; margin-bottom: 30px; }
        .page-header h1 { font-size: 2rem; color: #2c3e50; font-weight: 600; }
        .page-header h1 small { color: #7f8c8d; font-weight: 400; font-size: 1.5rem; }
        ...
        /* CSS untuk Status Pesanan & Pengiriman */
        ...
        .status-badge.sudah_sampai, .status-badge.selesai { background-color: #28a745; }
        ...
        .btn { display: inline-block; padding: 12px 25px; margin-left: 10px; font-size: 1rem; font-weight: 600; color: #ffffff; text-align: center; background-color: #3498db; border: none; border-radius: 8px; text-decoration: none; cursor: pointer; transition: all 0.3s ease; }
        .btn:hover { opacity: 0.9; }
        @media (max-width: 992px) { .main-content { margin-left: 0; } .sidebar { transform: translateX(-100%); } }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <div class="detail-grid">
            <div class="detail-card">
                ...
                <?php
                    ...
                    $status_pesanan_class = strtolower(str_replace(' ', '_', $overall_status));
                    $status_pembayaran_class = strtolower(str_replace('_', '_', $order_details['status_pembayaran']));
                ?>
                <p><strong>Status Pesanan:</strong> <span class="status-badge <?php echo $status_pesanan_class; ?>"><?php echo htmlspecialchars(ucwords(str_replace('_', ' ',$overall_status))); ?></span></p>
                <p><strong>Status Pembayaran:</strong> <span class="status-badge <?php echo $status_pembayaran_class; ?>"><?php echo htmlspecialchars(ucwords(str_replace('_', ' ', $order_details['status_pembayaran']))); ?></span></p>
        ...
        <div class="button-group">
            <?php 
                ...
                if ($role === 'admin' && !empty($order_details['pengiriman_id']) && $overall_status !== 'selesai' && $overall_status !== 'dibatalkan') {
                ...
                if ($role === 'sopir' && !empty($order_details['pengiriman_id']) && $overall_status !== 'selesai' && $overall_status !== 'dibatalkan') {
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            try {
                ...
                document.getElementById('delivery-map').innerHTML = "<p style='color:red;'>Gagal memuat peta.</p>";
            }
        });
        <?php endif; ?>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Script untuk sidebar dinamis (jika diperlukan)
            // Misalnya: tombol untuk membuka/menutup di mobile
        });
    </script>
</body>
</html>
